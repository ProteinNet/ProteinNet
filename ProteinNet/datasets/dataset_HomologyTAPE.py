...
class ProtClassHomologyTAPEDataset(Dataset):
    ...
    """
    def __init__(self, pDataSplit="training", pDataPath="data/HomologyTAPE", pMaxLen=1024, pSpaRad=8, pInterval=1.5, \
                pAddVirtual=False, pDebug=None):
        ...
        if not os.path.exists(f"{pDataPath}/{pDataSplit}_graph"):
            self.out_dir = f"{pDataPath}/{pDataSplit}_graph"
            self.dist_dir = f"{pDataPath}/{pDataSplit}_dist"
    ...
    def __getitem__(self, index):
        ...
        whole_prot_graph = torch.load(f"{self.pDataPath}/{self.pDataSplit}_graph/{targetCode}_whole.pt")
        # whole_prot_seq_  = torch.load(f"{self.pDataPath}/{self.pDataSplit}_graph/{targetCode}_seq.pt")
        ...
        # Add the label
        # whole_prot_seq.y = 
        whole_prot_graph.y = torch.tensor(targetLabel)
    ...
    def __preprocess__(self, pDataPath, pDataSplit):
        ...
        for (root, dirs, files) in os.walk(f"{pDataPath}/{pDataSplit}"):
            if len(files) > 0:
                for (i, file_name) in enumerate(tqdm(files)):
                    # Cache or garbage files.
                    if '.hdf5' not in file_name:
                        continue
                    pFileNameSplit = file_name.split('.')
                    queryname = pFileNameSplit[0]
                    if len(pFileNameSplit) > 2:
                        queryname = f"{pFileNameSplit[0]}.{pFileNameSplit[1]}"
                    ...
                    curProtein.load_hdf5(f"{pDataPath}/{pDataSplit}/{file_name}")
...
class ProtClassHomologyTAPEDatasetWrapper(object):
    ...
    def get_data_loaders(self):
        train_dataset           = ProtClassHomologyTAPEDataset(pDataSplit="training",           pDataPath=self.data_path, pMaxLen=self.max_seq, pAddVirtual=self.add_virtual, pInterval=self.interval)
        valid_dataset           = ProtClassHomologyTAPEDataset(pDataSplit="validation",         pDataPath=self.data_path, pMaxLen=self.max_seq, pAddVirtual=self.add_virtual, pInterval=self.interval)
        test_family_dataset     = ProtClassHomologyTAPEDataset(pDataSplit="test_family",        pDataPath=self.data_path, pMaxLen=self.max_seq, pAddVirtual=self.add_virtual, pInterval=self.interval)
        test_superfamily_dataset= ProtClassHomologyTAPEDataset(pDataSplit="test_superfamily",   pDataPath=self.data_path, pMaxLen=self.max_seq, pAddVirtual=self.add_virtual, pInterval=self.interval)
        test_fold_dataset       = ProtClassHomologyTAPEDataset(pDataSplit="test_fold",          pDataPath=self.data_path, pMaxLen=self.max_seq, pAddVirtual=self.add_virtual, pInterval=self.interval)
