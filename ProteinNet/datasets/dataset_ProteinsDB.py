...
class ProtClassProteinsDBDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Add the label
        # whole_prot_seq.y = 
        # whole_prot_graph.y = float(targetLabel)
    ...
    def __parse_dataset_(self):
        ...
        with open(f"{self.pDataPath}/amino_enzymes.txt", 'r') as mFile:
            for curLine in mFile:
                ...
                elif self.pDataSplit == 'validation' and curProtein in foldProteins:
                    targetCodes.append(curProtein)
                    targetLabels.append(1)
    ...
    def __get_edges__(self, aminoNeighsSparse, aminoNeighsHBSparse, distMatrix):
        ...
        '''
        aminoNeighsSparse=aminoNeighsSparse.type(torch.int64)
        aminoNeighsHBSparse=aminoNeighsHBSparse.type(torch.int64)
        ...
        for (root, dirs, files) in os.walk(f"{pDataPath}/data"):
            if len(files) > 0:
                for (i, file_name) in enumerate(tqdm(files)):
                    ...
                    curProtein.load_hdf5(f"{pDataPath}/data/{queryname}.hdf5")
    ...
    def __preprocess_debug__(self, pDataPath, pDataFold, pDebug):
        periodicTable_ = PyPeriodicTable()
        self.numAAs_ = len(periodicTable_.aLabels_) # default = 26
        curProtein = PyProtein(periodicTable_)
        curProtein.load_hdf5(f"{pDataPath}/data/{pDebug}.hdf5")
